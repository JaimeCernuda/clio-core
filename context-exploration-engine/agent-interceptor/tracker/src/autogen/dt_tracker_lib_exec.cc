...
#include "dt_provenance/tracker/autogen/dt_tracker_methods.h"
...
chi::TaskResume Runtime::Run(chi::u32 method, hipc::FullPtr<chi::Task> task_ptr,
  ...
  switch (method) {
    ...
    case Method::kStoreInteraction: {
      co_await StoreInteraction(task_ptr.template Cast<StoreInteractionTask>(), rctx); break;
    }
    case Method::kQuerySession: {
      co_await QuerySession(task_ptr.template Cast<QuerySessionTask>(), rctx); break;
    }
    case Method::kListSessions: {
      co_await ListSessions(task_ptr.template Cast<ListSessionsTask>(), rctx); break;
    }
    case Method::kGetInteraction: {
      co_await GetInteraction(task_ptr.template Cast<GetInteractionTask>(), rctx); break;
    }
    default: break;
  }
  co_return;
...
void Runtime::SaveTask(chi::u32 method, chi::SaveTaskArchive& archive,
  ...
  switch (method) {
    ...
    SAVE_CASE(kStoreInteraction, StoreInteractionTask);
    SAVE_CASE(kQuerySession, QuerySessionTask);
    SAVE_CASE(kListSessions, ListSessionsTask);
    SAVE_CASE(kGetInteraction, GetInteractionTask);
    default: break;
  }
}
...
void Runtime::LoadTask(chi::u32 method, chi::LoadTaskArchive& archive,
  ...
  switch (method) {
    ...
    LOAD_CASE(kStoreInteraction, StoreInteractionTask);
    LOAD_CASE(kQuerySession, QuerySessionTask);
    LOAD_CASE(kListSessions, ListSessionsTask);
    LOAD_CASE(kGetInteraction, GetInteractionTask);
    default: break;
  }
}
...
hipc::FullPtr<chi::Task> Runtime::NewCopyTask(
  ...
  switch (method) {
    ...
    COPY_CASE(kStoreInteraction, StoreInteractionTask)
    COPY_CASE(kQuerySession, QuerySessionTask)
    COPY_CASE(kListSessions, ListSessionsTask)
    COPY_CASE(kGetInteraction, GetInteractionTask)
    default: break;
  }
  return {};
}
...
hipc::FullPtr<chi::Task> Runtime::NewTask(chi::u32 method) {
  ...
  switch (method) {
    ...
    NEW_CASE(kStoreInteraction, StoreInteractionTask);
    NEW_CASE(kQuerySession, QuerySessionTask);
    NEW_CASE(kListSessions, ListSessionsTask);
    NEW_CASE(kGetInteraction, GetInteractionTask);
    default: return {};
  }
}
...
void Runtime::Aggregate(chi::u32 method, hipc::FullPtr<chi::Task> orig,
  ...
  switch (method) {
    ...
    AGG_CASE(kStoreInteraction, StoreInteractionTask);
    AGG_CASE(kQuerySession, QuerySessionTask);
    AGG_CASE(kListSessions, ListSessionsTask);
    AGG_CASE(kGetInteraction, GetInteractionTask);
...
void Runtime::DelTask(chi::u32 method, hipc::FullPtr<chi::Task> task_ptr) {
  ...
  switch (method) {
    ...
    DEL_CASE(kStoreInteraction, StoreInteractionTask);
    DEL_CASE(kQuerySession, QuerySessionTask);
    DEL_CASE(kListSessions, ListSessionsTask);
    DEL_CASE(kGetInteraction, GetInteractionTask);
